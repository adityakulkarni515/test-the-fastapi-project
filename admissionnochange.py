...
from sqlalchemy import create_engine, Column, Integer, String, Enum, Boolean, TIMESTAMP, ForeignKey, DECIMAL, Date, text, func
...
class Student(Base):
    ...
    created_at = Column(TIMESTAMP, server_default=text('CURRENT_TIMESTAMP'))
    admission_numbers = relationship("StudentAdmissionNumber", back_populates="student")

class StudentAdmissionNumber(Base):
    __tablename__ = "student_admission_numbers"
    id = Column(Integer, primary_key=True, index=True)
    student_id = Column(Integer, ForeignKey("students.student_id"), nullable=False)
    admission_number = Column(String, nullable=False, unique=True)
    start_date = Column(Date, nullable=False)
    end_date = Column(Date)
    student = relationship("Student", back_populates="admission_numbers")
...
# UPDATED: StudentDetails now fetches the current admission number
...
# UPDATED: Endpoint to get a student by their admission number
...
def get_student_by_admission_no(admission_no: str, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    # Query the new StudentAdmissionNumber table to find the student_id
    admission_number_record = db.query(StudentAdmissionNumber).filter(
        StudentAdmissionNumber.admission_number == admission_no
    ).first()

    if not admission_number_record:
        logger.warning(f"Student details request failed: Student with admission number {admission_no} not found.")
        raise HTTPException(status_code=404, detail="Student not found")

    student = admission_number_record.student
    
    # Get the current admission number for the student
    current_admission_number = db.query(StudentAdmissionNumber).filter(
        StudentAdmissionNumber.student_id == student.student_id,
        (StudentAdmissionNumber.end_date.is_(None) | (StudentAdmissionNumber.end_date >= func.current_date()))
    ).order_by(StudentAdmissionNumber.start_date.desc()).first()

    return StudentDetails(
        student_id=student.student_id,
        admission_number=current_admission_number.admission_number if current_admission_number else None,
...
# UPDATED: Student details now fetches the current admission number
